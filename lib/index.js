...
function screenMachine(config) {
  ...
  var router = (0, _routerFactory2.default)();
  ...
  var Component = components(document, events, machine, router);
  ...
  return {
    ...
    state: function state() {
      ...
      var path = registered.path;
      var paramTypes = registered.paramTypes;
    ...
    hasState: function hasState() {
      return machine.hasState.apply(machine, arguments);
    },
    href: function href() {
      return router.href.apply(router, arguments);
    },
    ...
    _navigateTo: function _navigateTo(url) {
      ...
      var stateArgs = router.find(url);
      if (stateArgs) {
        stateArgs.push(options);
        return this.transitionTo.apply(this, _toConsumableArray(stateArgs)).catch(function (err) {
